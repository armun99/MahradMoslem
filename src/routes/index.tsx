...
function Portfolio() {
  ...
  const selectedGallery = lightbox === null ? undefined : gallery[lightbox];
  ...
  return <main className="overflow-hidden bg-background text-foreground rock-texture">
    ...
    {selectedGallery && <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 p-4" role="dialog" aria-modal="true" aria-label="Gallery image"><Button variant="ghost" size="icon" onClick={()=>setLightbox(null)} className="absolute right-5 top-5 text-foreground" aria-label="Close gallery"><X /></Button><img src={selectedGallery[0]} alt={selectedGallery[2]} className="max-h-[86vh] max-w-[92vw] object-contain"/><p className="absolute bottom-5 text-xs uppercase tracking-widest text-muted-foreground">{selectedGallery[1]} — {selectedGallery[2]}</p></div>}
